...
//------------------------------------------------Creer table---------------------------------------------------------------    
    ...
    public int tailleEntrerAtt(Object[] val)
    {
        int h = 0;
        int i=4;
        while(val[i].equals(")") == false)
        {
            h++;
    ...
    public String[] EntrerAtt(Object[] val)
    {   
        int taille = this.tailleEntrerAtt(val);
        String[] valiny = new String[taille];
        int i = 4;
        int h = 0;
        while(val[i].equals(")") == false)
        {
            valiny[h] = (String)val[i];
            i++;
            h++;
        }
        return valiny;
...
//--------------------------------------------------Completer Donner-------------------------------------------------------
    public void Insert(String Nom,Object[] Valeur) throws Exception
    {
        File casa=new File(Nom+".txt");
        try(BufferedWriter ecrivain=new BufferedWriter(new FileWriter(casa,true)))
        {   
...
//Difference
        ...
        else if(reqUnite[0].equals("Create"))
        {
            String[] att = this.EntrerAtt(reqUnite);
            this.Create(reqUnite[2],att);
        }
        else if(reqUnite[0].equals("Insert"))
        {
            Object[] att = this.EntrerAtt(reqUnite);
            this.Insert(reqUnite[2],att);
        }
        else{
            RequetteException app = new RequetteException();
            System.out.println(app.ReqExc("Requette innexistante"));
        }
        return GrandTable;
    }
}
